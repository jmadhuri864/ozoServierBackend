...
export interface IAuthServiceInterface{
    ...
    logoutService:(token:string)=>Promise<any>;
    sendOTP:(emailAddress: string) =>Promise<any>;
    verifyOTP:(emailAddress: string, otp: string) => Promise<any>;
    resetPassword :(emailAddress: string,otp: string,newPassword: string) =>Promise<any>;
}